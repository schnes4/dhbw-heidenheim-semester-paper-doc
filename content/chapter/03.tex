...
Kommt es beispielsweise zu einem Messfehler - z.B. bedingt durch Lichteinflüsse - kann es passieren, dass ein Propellerblatt nicht erkannt respektive übersprungen wird.
...
Neben der Differenzierung zwischen infrarotem Lichtspektrum und sichtbarem Lichtspektrum bei der Messung, wiest der Lichtsensor eine sehr hohe Lux Reichweite auf.
\todo{Quelle?} 
\begin{figure}[H]
	\centering
...
Trotz der Tatsache, dass der Lichtsensor in den soeben beschriebenen Aspekten einem \ac{IR} deutlich überlegen ist, bietet ein \ac{IR} deutliche Vorteile, welche letztlich ausschlaggebend ist für die Entscheidung, welcher der beiden Sensoren verwendet werden soll.
Durch die technische Beschaffenheit des \ac{IR} im Vergleich zum Lichtsensor ist ein einfacherer Versuchsaufbau im Vergleich zum Lichtsensor möglich.
...
Dies hat zur Folge, dass am Prot des Arduinos 5V anliegen, solange der Sensor eine Reflexion des vom Emitter (In Abbildung \ref{fig:ir_example} durch ein E repräsentiert) ausgesendeten Lichtes erhält.
...
Im Gegensatz zum Lichtsensor ist der \ac{IR} anfällig gegenüber auf den Sensor einströmendes Licht.
...
Das größte Problem stellt die Kombination, der in Kapitel \ref{subsec:hardwaremodell} bewusst auf zwei Arduinos aufgeteilten Sensoren, bestehend aus einem \ac{9-DOF}, welcher in Kapitel \ref{subsec:detektion_der_unwucht} näher beleuchtet wird, und einem \ac{IR} an einem einzigen Arduino dar.
\begin{figure}[H]
	\centering
...
Des Weiteren werden teilweise auch Maximalwerte von 15V gemessen, welche aufgrund der Stromversorgung von maximal 5V nicht vorkommen können.

...
- Verwendeter Sensor für die Unwuchtdetektion ist der Beschleunigungssensor, da damit Vibrationen gut erkennbar sind.
- Erster Test der Unwuchtmessung um mögliche Muster zu erkennen:
-- A2 liest Interrupt und startet Messung
-- A2 misst weiter
-- A2 liest Interrupt und stoppt Messung
-- Dargestellt in Figure
- Tabellarische Darstellung der Messergebnisse 
-- aus den Whiteboard-Notes
- Zwei Dinge sind erkennbar
-- Maximalwert von 19.6 -> Deshalb erhöhung der Reichweite des Sensors, aber damit Verlust an Präzision
-- Keine Konsistenz zwischen den einzelnen Messreihen
--- Z-Wert ist beim 1. 0,66 -- beim 2. 3,9 -- beim 3. 3,4
- ABER: eine Unwucht ist bereits detektierbar anhand der Messergebnisse
- ABER: nicht lokalisierbar
-- Siehe "Umdrehungen ohne Unwucht" mit Vergleich zu "Umdrehungen mit Unwucht"

- Da Unwucht prinzipiell detektierbar kam folgender gedanke:
-- wenn unterschied messbar zwischen:
--- unwucht auf seite der schwarzen markierung
--- unwucht auf seite ohne markierung
-- dann ist unwucht auch lokalisierbar
- Messungen durchgeführt:
-- keine eindeutigen unterschiede der beiden unwuchten messbar
-- es kommt noch schlimmer: messreihen untereinander nicht einheitlich (siehe oben)

-> Unwucht nicht detektierbar mit diesem sensor

Erste Tabelle: Umdrehung mit Unwucht
\begin{table}[H]
\centering
\resizebox{\textwidth}{!}{%
...
2       & 16   & -19,60 & -5,30 & -0,28 & 16   & -19,60 & -5,29 & -0,25 & 17   & -19,6  & -4,68 & 0,71  \\
...
Zweite Tabelle: Umdrehung ohne Unwucht
\begin{table}[H]
\centering
\resizebox{\textwidth}{!}{%
\begin{tabular}{lllllllllllll}
...
Tabelle: Unwucht auf Seite mit Markierung
\begin{table}[H]
\centering
\resizebox{\textwidth}{!}{%
\begin{tabular}{lllllllllllll}
...
Tabelle: Unwucht auf Seite ohne Markierung
\begin{table}[]
\centering
\resizebox{\textwidth}{!}{%
\begin{tabular}{lllllllllllll}
